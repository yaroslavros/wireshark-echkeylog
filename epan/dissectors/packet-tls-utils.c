...
ssl_dissect_hnd_ech_outer_ext(ssl_common_dissect_t *hf, tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree,
                              guint32 offset, guint32 offset_end)
{
    guint32     ext_length, next_offset;
    proto_tree *ext_tree;
    proto_item *ti;
...
ssl_dissect_hnd_hello_ext_ech(ssl_common_dissect_t *hf, tvbuff_t *tvb, packet_info *pinfo,
                              proto_tree *tree, guint32 offset, guint32 offset_end,
                              guint8 hnd_type, SslSession *session, SslDecryptSession *ssl, ssl_master_key_map_t *mk_map,
                              guint32 initial_offset, guint32 hello_length)
{
    ...
    case SSL_HND_CLIENT_HELLO:
        ...
        case 0: /* outer */
            ...
            guint16 kdf_id = tvb_get_ntohs(tvb, offset - 4);
            guint16 aead_id = tvb_get_ntohs(tvb, offset - 2);

            proto_tree_add_item(tree, hf->hf.ech_config_id, tvb, offset, 1, ENC_BIG_ENDIAN);
            guint8 config_id = tvb_get_guint8(tvb, offset);
            offset += 1;
            ...
            guint8 kdf_len = hpke_hkdf_len(kdf_id);
            if (kdf_len == 0) {
                ssl_debug_printf("Unsupported KDF\n");
                break;
            }

            guint8 aead_key_len = hpke_aead_key_len(aead_id);
            if (aead_key_len == 0) {
                ssl_debug_printf("Unsupported AEAD\n");
                break;
            }

            guint8 aead_nonce_len = hpke_aead_nonce_len(aead_id);

            guint16 version = GUINT16_FROM_BE(*(guint16 *)ech_config->data);
            if (version != SSL_HND_HELLO_EXT_ENCRYPTED_CLIENT_HELLO) {
                ssl_debug_printf("Unexpected version in ECH Config\n");
                break;
            }
            guint32 ech_config_offset = 2;
            if (GUINT16_FROM_BE(*(guint16 *)(ech_config->data + ech_config_offset)) != ech_config->data_len - 4) {
            ...
            ech_config_offset += 1;
            guint16 kem_id_be = *(guint16 *)(ech_config->data + ech_config_offset);
            guint16 kem_id = GUINT16_FROM_BE(kem_id_be);
            ...
            guint8 key[AEAD_MAX_KEY_LENGTH];
            guint8 base_nonce[HPKE_AEAD_NONCE_LENGTH];
            ...
            const guint8 *payload = tvb_get_ptr(tvb, offset - length, length);
            guint8 *ech_aad = (guint8 *)wmem_alloc(NULL, hello_length);
            ...
            wmem_free(NULL, ech_aad);
            guint8 *ech_decrypted_data = (guint8 *)wmem_alloc(pinfo->pool, length - 16);
            ...
            if (memcmp(pinfo->num > session->first_ch_ech_frame ? ech_auth_tag_calc : session->first_ech_auth_tag,
                ...
                if (ssl) {
                    ...
                    guint32 len_offset = ssl->ech_transcript.data_len;
                    ...
                    tvb_memcpy(tvb, ssl->ech_transcript.data + ssl->ech_transcript.data_len, initial_offset + 34,
                        tvb_get_guint8(tvb, initial_offset + 34) + 1);
                    ssl->ech_transcript.data_len += tvb_get_guint8(tvb, initial_offset + 34) + 1;
                    guint32 ech_offset = 35 + tvb_get_guint8(ech_tvb, 34);
                    ...
                    tvb_memcpy(ech_tvb, ssl->ech_transcript.data + ssl->ech_transcript.data_len, ech_offset,
                               1 + tvb_get_guint8(ech_tvb, ech_offset));
                    ssl->ech_transcript.data_len += 1 + tvb_get_guint8(ech_tvb, ech_offset);
                    ech_offset += 1 + tvb_get_guint8(ech_tvb, ech_offset);
                    guint32 ech_extensions_len_offset = ssl->ech_transcript.data_len;
                    ssl->ech_transcript.data_len += 2;
                    guint16 extensions_end = ech_offset + tvb_get_ntohs(ech_tvb, ech_offset) + 2;
                    ech_offset += 2;
                    while (extensions_end - ech_offset >= 4) {
                        if (tvb_get_ntohs(ech_tvb, ech_offset) != SSL_HND_HELLO_EXT_ECH_OUTER_EXTENSIONS) {
                            ...
                            guint8 outer_extensions_end = tvb_get_guint8(ech_tvb, ech_offset + 4) + ech_offset + 5;
                            ech_offset += 5;
                            guint16 outer_offset = initial_offset + 35 + tvb_get_guint8(tvb, initial_offset + 34);
                            outer_offset += tvb_get_ntohs(tvb, outer_offset) + 2;
                            outer_offset += tvb_get_guint8(tvb, outer_offset) + 3;
                    ...
                    guint16 ech_extensions_len_be = GUINT16_TO_BE(ssl->ech_transcript.data_len - ech_extensions_len_offset - 2);
                ...
                guint32 ech_padding_begin = (guint32)ssl_dissect_hnd_cli_hello(hf, ech_tvb, pinfo, payload_tree, 0, length - 16, session,
                                                                               ssl, NULL, mk_map);
...
ssl_dissect_hnd_extension(ssl_common_dissect_t *hf, tvbuff_t *tvb, proto_tree *tree,
                          ...
                          ssl_master_key_map_t *mk_map, guint32 initial_offset, guint32 hello_length);
gint
                          ...
                          SslDecryptSession *ssl, dtls_hfs_t *dtls_hfs, ssl_master_key_map_t *mk_map)
{
    ...
    guint32     hello_length = offset_end - initial_offset;
    ...
    if (ssl && ssl->ech_transcript.data_len > 0 && (ssl->state & SSL_CIPHER) && ssl->client_random.data_len > 0) {
        ...
        if (hash_algo) {
            ...
            if (initial_offset > 4) {
                ...
                guint8 prefix[4] = {SSL_HND_SERVER_HELLO, 0x00, 0x00, 0x00};
            ...
            ssl->ech_transcript.data_len += 4 + offset_end - initial_offset;
            guint8 zeros[8] = { 0 };
            guint32 confirmation_offset = initial_offset + 26;
            if (is_hrr) {
                guint32 hrr_offset = initial_offset + 34;
                ssl_md_update(&mc, (guchar *)tvb_get_ptr(tvb, hrr_offset,
                                             tvb_get_guint8(tvb, hrr_offset) + 1), tvb_get_guint8(tvb, hrr_offset) + 1);
                hrr_offset += tvb_get_guint8(tvb, hrr_offset) + 1;
                ssl_md_update(&mc, (guchar *)tvb_get_ptr(tvb, hrr_offset, 3), 3);
                hrr_offset += 3;
                guint16 extensions_end = hrr_offset + tvb_get_ntohs(tvb, hrr_offset) + 2;
